...
SELECT U.APELLIDO, U.NOMBRE, T.ID,  
        ( SELECT COUNT(*) FROM VIAJES V INNER JOIN MOVIMIENTOS_TARJETAS MOV ON MOV.ID = V.IDMOVIMIENTOTARJETA WHERE MOV.IDTARJETA = T.ID  ) AS CANT_VIAJES, 
        ( SELECT COUNT(*) FROM MOVIMIENTOS_TARJETAS MOV_TARJ WHERE MOV_TARJ.IDTARJETA = T.ID AND MOV_TARJ.TIPOMOVIMIENTO='C') AS CANTIDAD_RECARGAS,  
        ( SELECT SUM( CASE  MOV.TIPOMOVIMIENTO 
            WHEN 'C' THEN MOV.IMPORTE
            ELSE (MOV.IMPORTE*-1)           
        END) FROM MOVIMIENTOS_TARJETAS MOV  WHERE MOV.IDTARJETA = T.ID  ) AS TOTAL_ACREDITADO, 
        ( SELECT AVG(MOV_TARJ.IMPORTE) FROM MOVIMIENTOS_TARJETAS MOV_TARJ WHERE MOV_TARJ.IDTARJETA = T.ID AND MOV_TARJ.TIPOMOVIMIENTO='C') AS IMPORTE_RECARGA_PROMEDIO, 
        T.ESTADO
FROM TARJETAS T
